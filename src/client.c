...
if ((client1->acc_status) && (client2->acc_status) && (balance <= (client1->acc_balance))){
...
}else{printf(" Sorry!\n Account ID %d has not sufficient balance or\n any of account ID %d or %d is not active",id1, id1, id2);}
}
...
void set_account_status(u32 id, u8* status){
	sClient_t* client = get_client(id);
	if (client && strcmp("active", status) == 0){	
